    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg max-h-[70vh] overflow-y-auto">
        <table class="table-auto min-w-full divide-y divide-gray-200 dark:divide-gray-600 text-center" id="roles_table" wire:poll.10s="autoUpdate">
            <thead class="bg-gray-100 dark:bg-gray-700 sticky top-0 z-10">
                <tr>
                    @foreach (['id' => 'ID', 'subject_name' => 'Subject Name', 'actions' => 'Actions'] as $field => $label)
            ...
            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                @forelse ($dataSubject as $subject)
                    <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 odd:bg-white odd:dark:bg-gray-900 even:bg-gray-100 even:dark:bg-gray-800 border-b dark:border-gray-700">
                        <td class="p-1 text-sm font-normal text-gray-500 dark:text-gray-400 text-center break-all">{{ $subject->id }}</td>
                        <td class="py-3 text-sm font-normal text-gray-500 dark:text-gray-400 text-center break-all">{{ $subject->subject_name }}</td>
                        <td class="p-3 space-x-2 whitespace-nowrap text-center">
                            {{-- <button
                                class="read-more-subject-btn text-white bg-blue-500 hover:bg-blue-600 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-4 py-2 dark:focus:ring-blue-900"
                                data-id="{{ $subject->id }}" data-subject_name="{{ $subject->subject_name }}">
                                Read More
                            </button> --}}
                            <button
                                class="edit-subject-btn text-white bg-yellow-500 hover:bg-yellow-600 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-4 py-2 dark:focus:ring-blue-900"
                                data-id="{{ $subject->id }}" data-subject_name="{{ $subject->subject_name }}">
                                Edit
                            </button>
                            <button
                                class="delete-subject-btn text-white bg-red-600 hover:bg-red-600 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-4 py-2 dark:focus:ring-blue-900"
                                data-id="{{ $subject->id }}" data-subject_name="{{ $subject->subject_name }}">
                                Delete
                            </button>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="py-1.5 px-4 text-center text-sm text-gray-500 dark:text-gray-400">No subjects found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
