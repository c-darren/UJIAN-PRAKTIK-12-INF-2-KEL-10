    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg max-h-[70vh] overflow-y-auto">
        <table class="table-auto min-w-full divide-y divide-gray-200 dark:divide-gray-600 text-center" wire:poll.10s="autoUpdate">
            <thead class="bg-gray-100 dark:bg-gray-700 sticky top-0 z-10">
                <tr>
                    @foreach (['id' => 'ID', 'StudentName' => 'Student Name', 'EnrolledStatus' => 'Status', 'actions' => 'Actions'] as $field => $label)
            ...
            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                @forelse ($records as $record)
                    <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 odd:bg-white odd:dark:bg-gray-900 even:bg-gray-100 even:dark:bg-gray-800 border-b dark:border-gray-700">
                        ...
                        <td class="py-3 text-sm font-normal text-gray-500 dark:text-gray-400 text-center break-all">{{ $record->name }}</td>
                        <td class="py-3 text-sm font-normal text-center break-all
                            {{ ($record->status == 'Enrolled') ? "text-green-500" : "text-red-500" }}">
                            {{ $record->status }}
                        </td>
                        <td class="p-3 space-x-2 whitespace-nowrap text-center">
                            <button
                                ...
                                data-action="{{ route('master_class_students.delete', [$masterClass_id, $record->id]) }}"
                                data-col_01="{{ $record->name }}">
                                Delete
                            </button>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="4" class="py-1.5 px-4 text-center text-sm text-gray-500 dark:text-gray-400">No records found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
