        ...
        <div class="max-w-md w-full">
            ...
            <div class="relative">
                ...
                <input 
                    ...
                    placeholder="Search by Academic Year or Status" 
                />
                <!-- Tombol Search -->
                <button 
                    type="button" 
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg max-h-[70vh] overflow-y-auto">
        <table class="table-auto min-w-full divide-y divide-gray-200 dark:divide-gray-600 text-center" id="roles_table" wire:poll.10s="autoUpdate">
            <thead class="bg-gray-100 dark:bg-gray-700 sticky top-0 z-10">
                <tr>
                    @foreach (['id' => 'ID', 'academic_year' => 'Academic Year', 'status' => 'Status', 'actions' => 'Actions'] as $field => $label)
            ...
            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                @forelse ($dataAcademicYear as $academicYear)
                    <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 odd:bg-white odd:dark:bg-gray-900 even:bg-gray-100 even:dark:bg-gray-800 border-b dark:border-gray-700">
                        <td class="p-1 text-sm font-normal text-gray-500 dark:text-gray-400 text-center break-all">{{ $academicYear->id }}</td>
                        <td class="py-3 text-sm font-normal text-gray-500 dark:text-gray-400 text-center break-all">{{ $academicYear->academic_year }}</td>
                        <td class="py-3 text-sm font-normal {{ $academicYear->status == 'Inactive' ? 'text-red-600' : 'text-green-600' }}  text-center break-all">{{ $academicYear->status }}</td>
                        <td class="p-3 space-x-2 whitespace-nowrap text-center">
                            <button
                                class="read-more-academic-year-btn text-white bg-blue-500 hover:bg-blue-600 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-4 py-2 dark:focus:ring-blue-900"
                                data-id="{{ $academicYear->id }}" data-academic_year="{{ $academicYear->academic_year }}" data-status="{{ $academicYear->status }}" data-created_at="{{ $academicYear->created_at }}" data-updated_at="{{ $academicYear->updated_at }}">
                                Read More
                            </button>
                            <button
                                class="edit-academic-year-btn text-white bg-yellow-500 hover:bg-yellow-600 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-4 py-2 dark:focus:ring-blue-900"
                                data-id="{{ $academicYear->id }}" data-academic_year="{{ $academicYear->academic_year }}" data-status="{{ $academicYear->status }}">
                                Edit
                            </button>
                            <button
                                class="delete-academic-year-btn text-white bg-red-600 hover:bg-red-600 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-4 py-2 dark:focus:ring-blue-900"
                                data-id="{{ $academicYear->id }}" data-academic_year="{{ $academicYear->academic_year }}">
                                Delete
                            </button>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="4" class="py-1.5 px-4 text-center text-sm text-gray-500 dark:text-gray-400">No academic years found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
