<div x-show="$store.deleteMaterialModal.open"
    x-data="deleteMaterialModalData()"
    x-cloak
    class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-75 overflow-y-auto"
    @keydown.escape.window="$store.deleteMaterialModal.close()"
    ...
    <div @click.away="$store.deleteMaterialModal.close()"
        ...
        <div class="sticky top-0 bg-white dark:bg-gray-800 z-30 p-4 border-b border-gray-200 dark:border-gray-700 shadow-md">
            <div class="flex justify-between items-center">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Delete Material Form</h3>
                <button @click="$store.deleteMaterialModal.close()" class="text-gray-400 hover:text-gray-900 dark:hover:text-white">
        ...
        <form class="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 max-h-[70vh] overflow-y-auto" id="deleteForm" x-ref="deleteForm">
            @csrf
            <input type="hidden" name="actionUrl" x-model="$store.deleteMaterialModal.data.actionUrl">

            <div class="md:col-span-2">
                <label class="block font-medium text-gray-900 dark:text-white mb-3 text-lg">
                    Do you really want to delete <span class="font-bold" x-text="$store.deleteMaterialModal.data.col_01"></span>? Please confirm.<br>
                </label>
            </div>
        </form>
        ...
        <div class="flex items-center justify-end space-x-2 border-t bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 pt-4 sticky bottom-0">
            <button type="button" @click="$store.deleteMaterialModal.close()" class="bg-gray-500 text-white px-4 py-2 rounded">Cancel</button>
